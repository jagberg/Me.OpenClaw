...
- Not built yet (Phase 2): `revert_state`, the claim timeline, and the projection being authoritative. Until then the column is what everything reads, and the projection only watches. `_reverted_event_ids` already skips reverted events but handles one level only — reverting a reversion is Phase 2's.
