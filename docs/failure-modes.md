...
**Read it first — before the work, not after.** Any task whose premise is a claim about *what the real data contains* ("the cache is stale", "no invoice carries X", "this never happens") starts by searching this file. That is what it is for, and on 2026-07-31 it was skipped: a re-extraction was run and its central finding reported as new when row 37 had recorded it two days earlier. The tokens were spendable; the rediscovery was not. An index nobody reads is a sweep nobody saved.
...
| **This index was not consulted before spending tokens to rediscover a row already in it.** Row 37 recorded "3 line-item dates found, all equal to their invoice header date" on 2026-07-29; a re-extraction on 2026-07-31 reported that same fact as a new discovery, overstated it, and corrected back to the original wording | this file, `256f6ae`; corrected in `vet-request-document-and-monday-nudge` tasks.md 3b | **No check yet, and it is the one that would pay for itself.** Read this file *first* when a task's premise is about what the real data contains. `eval-change` axis 3(c) catches it only after the work is done |
...
| Same shape, live, 2026-08-04: Groq began refusing this network (403 to an **unauthenticated** request), taking the primary and all three fallbacks at once. Switching providers was manual, and the agent ran with no model for four deploys | ADR-0009 amendment (2026-08-04) | Nothing automatic. `test_chat_has_a_gemini_backend_and_the_agents_primary_is_the_reachable_provider` only asserts the *current* choice; no check notices a provider going unreachable |
...
| The inbound half of `telegram_messages` stopped being written at the cutover and nobody noticed for a day. Task 4.2 was ticked for registering a hook, while the event tee named in its own sentence was never wired — so six live commands and taps produced zero inbound rows, the replay queue is empty by construction, and "did my tap register?" is answerable only from container logs, which is what ADR-0014 exists to avoid | task 4.2 / 4.12 (2026-08-04) | **None.** A ticked task and a working system looked identical. Nothing asserts that an inbound row exists per command — and a unit test cannot, because the writer is the plugin |
| A one-shot reminder job lived only in APScheduler's in-memory jobstore, and nothing re-registered pending ones at startup — so a restart silently dropped every reminder not yet fired. `misfire_grace_time=None` promised restart-safety and could not deliver it: after a restart there was no job left to misfire. Two live reminders sat unfired for two days | task 5.2 (2026-08-04) | Fixed by construction — `reminders.sweep_due` asks the DB what is due instead of remembering what it meant to do; `test_a_reminder_due_while_the_app_was_down_still_fires_and_says_how_late` |
...
3. **The fallback chain is still single-provider — and this one has now fired for real.** 2026-08-04: Groq began refusing this network entirely (403 to a request with no auth header), so the primary and all three fallbacks died in one move, and the gateway agent had no model for four deploys. The cure was manual: switch `LLM_PROVIDER` and the gateway's primary to Gemini (ADR-0009's amendment). Gemini now has its own four-model chain, so the gap is *unchanged in kind* — two chains, each one provider deep, and nothing fails over between them.
