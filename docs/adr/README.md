...
| [0025](0025-the-proposal-gate-is-split-by-origin.md) | The proposal gate is split by origin and its text is composed by code — card taps commit behind /internal, chat proposals in the MCP surface, neither from the model's own account. **Amended 2026-08-02: the MCP half has no mechanism** — the gateway's MCP client declares no `elicitation` capability, so the server cannot gate its own commit; the split collapses to one commit path. **Gate location superseded by 0027** (Justin, 2026-08-02); everything else here stands | accepted (gate location superseded by 0027) | 2026-08-01 |
...
| [0027](0027-the-proposal-gate-has-one-commit-path.md) | The proposal gate has one commit path, reached from two entry points — supersedes 0025's gate *location* only; the MCP surface cannot gate its own commit because the gateway's MCP client declares no `elicitation` capability | accepted | 2026-08-02 |
| [0028](0028-groqs-403-was-a-user-agent-ban.md) | Groq's 403 was a Cloudflare User-Agent ban on the *probe*, not a network block — both runtimes reach it with their own clients; supersedes the finding of fact in 0009's 2026-08-04 amendment, not its decision. Probe with the client the code uses. No config changed; the cross-provider fallback 0017/0026 want is now buildable | accepted | 2026-08-08 |
| [0029](0029-gateway-plugin-hooks-dont-fire-commands-do.md) | A plugin's `before_dispatch`/`message_received` hooks never invoke their handler in gateway 2026.7.1 (the dispatcher drops the `ctx` argument the wrapper needs) — commands are the only proven dispatch path. Casts doubt on `claims-pending-flow`'s condition-entry flow too, unverified since the cutover by the same mechanism | accepted | 2026-08-10 |
| [0030](0030-vet-invoice-request-emails-are-sent-not-drafted.md) | Vet invoice-request emails are sent directly, not drafted — one named, narrow exception to CLAUDE.md's "never send email" hard rule, scoped to this call site only | accepted | 2026-08-11 |
