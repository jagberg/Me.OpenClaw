...
- **The data dir is shared; `.env` is NOT.** Compose binds `C:/code/Me.OpenClaw/app/data:/data` deliberately (one DB, not two) but resolves `env_file: ./app/.env` **relative to the deploy worktree** — so the running container reads `C:\Code\Me.OpenClaw-telegram-claimquery\app\.env`, a different file from the main checkout's. Found 2026-07-31: the container has `LLM_PROVIDER=groq` and a `GROQ_API_KEY`; the main checkout's `.env` has **neither**, only Gemini. Consequence to expect: anything run from the main checkout has no Groq at all and will look like a provider outage, and editing one `.env` changes nothing about the other. Whether this divergence is deliberate is **unrecorded** — it may simply be that the worktree's copy was updated and the main one never was.
