...
<!--
The four of this capability's ten requirements that only become true once the
gateway holds the bot token. The other six shipped and archived with
`openclaw-gateway-core` (slice 1) and are already in `openspec/specs/`; the
split and its reasoning are recorded in that change's task 8.11.
-->
