...
- [~] 4.2 **PARTLY DONE, and the tick was wrong until 2026-08-04.** What exists: the plugin registers `before_dispatch` via `api.registerHook` and forwards to `/internal/telegram/claim`; it holds no claims logic and fails open, so a failed check sends the message to the agent rather than losing it. Observed claiming a live message — Justin's typed sentence on 2026-08-04 reached the agent and was acked.
  ...
  **Update 2026-08-05 — the paragraphs above overstate the gap, and were left standing for a day after the code closed part of it (found by review, not by me).** The "choice" they describe as unmade was made in `e1d0801`/`912d6a9`: `internal_api.tee_inbound` writes the row with a synthetic id (`cmd:<correlation>:<random>`), so commands, taps and claimed pending-flow text now log an inbound row. "Zero inbound rows" is no longer true — verified live 2026-08-04 11:39:40, `in cmd:tg-history-n1 kind=command corr=tg-history-n1 /history`, the first inbound row since 2026-08-02.

  What is still genuinely missing, narrower than the text above:
  - No tee for raw `edited_message` or for a callback query that is not a registered command. Those never enter the plugin's command handler at all.
  - The row's id is synthetic, so **dedupe and replay remain off** for these rows: a Telegram redelivery writes a second row where a real `update_id` would have collided. `record_inbound_raw`'s own docstring says the dedupe never gated handling, so this costs dataset tidiness, not behaviour. 4.12 is still unverifiable — nothing queues.
  - A pre-cutover tap logged `kind=tap` with callback data (`invreq:4`); the same tap now logs `kind=command` with the command text. ADR-0014's "rows indistinguishable across the swap" is not met for taps, and cannot be on this transport — the gateway delivers a tap as a command.
...
- [~] 9.4 **PARTLY DONE 2026-08-06 — done for what this transport can deliver, and the rest is blocked by 4.2, not by effort.** `test_the_gateway_row_and_the_ptb_row_differ_in_nothing_a_query_reads` writes one row through each transport from the same text and compares them column by column: `direction`, `kind`, `summary` and `app_version` must be equal, none of them NULL, `app_version` must equal `config.APP_VERSION`, the payload must still parse and still carry text/username/chat id, and `processed_at` must be NULL until settled and never earlier than `received_at`. Two columns legitimately differ and both are recorded in 4.2: the synthetic `update_id`, and a payload that is thinner (no `message_id`, no `date`, no entities — the plugin's ctx carries none of them).

  The edit half of this task's own sentence **cannot be asserted end to end**: `_describe` classifies an `edited_message` correctly and the test pins that (the 2026-07-27 regression guard), but nothing on the gateway path can produce one, because the tee builds a single `{"message": ...}` shape. Same root cause as 4.2's missing raw-event tee. The test asserts that gap explicitly — it fails if a payload ever gains `callback_query` or `edited_message`, which is the signal to finish this task rather than a signal something broke.

  Original task text, kept: Keep `telegram_messages` writing on the gateway path with no field lost — raw payload, truthful kind, summary, `app_version`, `processed_at` ordering. Assert an edit event still logs as an edit with its text (the 2026-07-27 empty-`other` regression).
...
- [x] 9.6 **DONE 2026-08-06.** Every `/internal/*` run now logs `duration_ms=` and `changed=` — a delta of the claim status histogram plus counts of flagged claims, status events, outbound messages, tasks, scheduled reminders, LLM calls and ops alerts. Both are in the JSON response as well as the log line. Also on the **failure** path: a tick that died after 40s and one that died on its first line need different diagnoses, and a partial tick's effects are what the next reader needs.

  Counted before and after rather than reported by each step, deliberately: a step that forgets to report is indistinguishable from a step that did nothing, which is the exact failure being measured. `changed={}` is an answer, not an absent field — "ran and moved nothing" is the state worth alerting on later, and a missing key cannot be alerted on. A snapshot that itself fails returns `{}` and logs WARNING rather than inventing movement.

  Not persisted to `job_runs`: a column there needs hand-run `ALTER TABLE` against the live DB, and the log line plus the response answers the question. `test_a_tick_reports_what_it_did_not_merely_that_it_fired` covers the busy tick, the quiet tick, the direction of a status move (-1/+1 across two statuses) and the failed-snapshot case. The error path's log line is exercised but not asserted.

  Original task text, kept: Log each tick's outcome app-side (claims advanced, flags written, duration) rather than relying on `cron runs` alone; `cron runs` says it fired, not what it did.
