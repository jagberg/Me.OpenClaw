...
- [ ] 4.2 **Storing Petcover's serial → treatment-date table.** Their 2026-07-29 reply is the authority for the whole map and lives only in the mailbox. A `petcover_serials` table would be a *new* table, so `CREATE TABLE IF NOT EXISTS` covers it without hand-run DDL — but nothing yet asks Petcover for that table on a schedule, so it would be a snapshot that silently ages.
