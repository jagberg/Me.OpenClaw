...
- [~] 5.6 `pipeline.nudge_stale_actions`: include info requests regardless of `ACTION_NUDGE_DAYS` and name the one closest to its deadline — **SUPERSEDED 2026-07-31** by `vet-request-document-and-monday-nudge`. Folding info requests into the *daily* nudge was the wrong beat: a vet does not reply overnight, so a daily line about the same unanswered request is noise that trains Justin to ignore the nudge. It ships instead as a separate **weekly** job (`pipeline.nudge_unanswered_vet_requests`, `VET_NUDGE_DAY`, default Monday), silent when nothing is outstanding. `nudge_stale_actions` is unchanged and still daily. Trade-off accepted: two nudge jobs with different cadences to keep straight — noted in `app/openclaw/CLAUDE.md` so they are not merged back together by someone tidying up.
...
- [x] 6.3 Extract the requested document by regex from the letter (`we need a copy of …` / `please provide the following …`) — no LLM; leave it null when the text isn't there rather than guessing — **ABSORBED and shipped** by `vet-request-document-and-monday-nudge` group 1 as `claim_status.extract_requested_document`, deployed and verified live on claim #8 (`Consultation notes dated 18/05/2026`). Marked here 2026-07-31: that change's task 5.1 said to record it and did not, so this read as outstanding work for three days while the code was already in production. Its date-parsing and multi-item capture went beyond what this task asked for.
