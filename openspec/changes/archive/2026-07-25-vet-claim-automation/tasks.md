...
- [x] 1.1 Confirm manual-CSV workflow is acceptable, or pick an alternative (SMS-forwarding automation / paid aggregator) — **closed by practice, not by a decision record**: manual CSV upload shipped, is in use, and the no-credentials constraint was promoted to a hard rule in CLAUDE.md. The two alternatives (SMS-forwarding, paid aggregator) were never evaluated in writing. Recorded in the `bank-transaction-feed` baseline.
...
- [x] 6.0 [Blocked on Justin] Echo/Bow Wow Insurance claim process — template format, submission method (email vs portal), required fields all unknown until Justin clarifies with them; claim-form automation only branches to Petcover's path for now, Echo claims stay at `matched` with a "process not yet defined" flag — **moved to `openspec/BACKLOG.md`**, not completed. Kept out of this change so a shipped change isn't held unarchived by an item no code can close.
