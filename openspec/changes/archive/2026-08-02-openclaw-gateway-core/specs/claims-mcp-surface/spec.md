...
<!--
The read surface — five of this capability's eight requirements. They are true
the moment the tools are registered, because the gateway agent can be driven
through `openclaw agent` with no channel bound, which is how they were verified.

The three that need a real tap on a real button (the proposal gate, the harness
refusals it carries, and mutating-tool claim identity) move to
`openclaw-telegram-cutover`. Task 8.11 records why splitting this capability was
preferred to holding it whole.
-->
