...
<!--
Six of this capability's ten requirements — the ones that are true while the
Python app still owns Telegram. The other four (channel transport, outbound via
gateway actions, independent degradation, gateway cron) move to
`openclaw-telegram-cutover`, because each describes a system that does not exist
until the gateway holds the bot token. Task 8.11 records the boundary rule and
the per-requirement assignment.
-->
