...
- [x] 1.4 **DONE** — built as the logging tee (12.1), guarded, with the redelivery case asserted. Add `POST /internal/telegram/event` accepting a gateway-shaped event; call `message_log.record_inbound` **before** any handler runs.
...
- [x] 7.0a-orig **DONE** — see the combined 7.0a/7.1-7.4 entry above. The gateway container gets **no bind mount to `app/data`** and no `DATABASE_PATH`. This is `gmail-isolation-boundary` enforced by the container boundary rather than by configuration — the Gmail token, the SQLite file and the invoice PDFs are simply not on its filesystem. The strongest form of D4, and the isolation reason is why.
- [x] 7.0b **DONE differently, and better: a fixed compose subnet with static addresses**, because the allowlist matches exactly rather than by CIDR. The bridge-subnet framing assumed a dynamic address; pinning them removes the guesswork. Original: Set `INTERNAL_API_ALLOW_HOSTS` to the Docker bridge subnet the gateway actually arrives from. The loopback default works in neither deployment shape; the shared secret remains the real auth.
- [x] 7.0c **DONE for the volume; the backup question stays open** — `gateway_state` is named and outside `db_backup`'s scope, recorded in `docs/gateway-deploy.md` as costing session history rather than claim state. Original: Give the gateway container its own named volume for gateway state, and decide whether it joins `db_backup`'s scope. Its state directory currently sits outside every backup.

- [x] 7.1 **DONE.** Add the gateway as a second compose service on Node 24; keep the DB out of its config and out of any workspace it can see.
- [x] 7.2 **DONE.** Extend `scripts/deploy.ps1` to bring up both runtimes, stamp and report both versions, and print each health check.
- [x] 7.3 **DONE** — deploy.ps1 collects per-runtime failures and throws naming them. Make a partial start report failure naming which runtime is down.
- [x] 7.4 **DONE** — `GATEWAY_VERSION` is stamped from the running image and surfaced beside `app_version` on `/health`, never into it. Keep `app_version` in `telegram_messages` as the Python app's version; record the gateway version separately without displacing it.
- [x] 7.5 **DONE and asserted every deploy** (19b.5), plus documented in `docs/gateway-deploy.md`. The spike container was recreated with no Google key at all. Original: Confirm the gateway holds no Gmail credential and no Gmail-scoped Google key; document where its state directory lives and whether it is backed up.
- [x] 7.6 **APPLIED, not just asserted (2026-08-02).** `deploy.ps1` now disables each boundary plugin whose config does not already say `false`, restarts the gateway when anything changed, and only then runs the preflight — which re-checks independently rather than restating what the deploy just did. The restart matters: enablement is read at startup, so a `config set` without one leaves the plugin **running** while config claims otherwise, and that divergence is exactly why the preflight reads the running set as well. Original: **Pin the plugin set — and the work is DISABLING, not avoiding.** Measured 2026-08-01 on a stock container: **47 of 66 plugins enabled by default**, and *every* boundary-relevant one is on — `browser`, `file-transfer`, `phone-control`, `canvas`, `device-pair`, `memory-core`, `talk-voice`. The design assumed the agent starts with no browser and no filesystem reach; the true default is the opposite. `gmail-isolation-boundary` therefore requires positive action to establish, not merely restraint in adding things later.
...
- [x] 7.7 **ANSWERED 2026-08-02, and it splits: the capability survives, the announcement does not.**

  The gateway ships a `health-monitor` (started at boot: `interval: 300s, startup-grace: 60s, channel-connect-grace: 120s`). On a stale channel it logs `health-monitor: restarting (reason: …)` and restarts it, rate-limited by `maxRestartsPerHour` — configurable as `gateway.channelMaxRestart*` and `gateway.channelStaleEventThresholdMinutes`. So ADR-0015's *restart* behaviour is native and does not need rebuilding; a silently dead poller still gets restarted.

  **What is lost is the loudness, which was the point of ADR-0015.** The app raised an `ops_alerts` row Justin could see. The gateway writes a line to a log that lives **inside its container** and is destroyed on recreate. A channel that died, restarted five times and hit the hourly cap would leave no trace anywhere Justin looks.

  **Consequence, and it is app-side work rather than gateway config:** the app must poll the gateway's health and raise an `ops_alert` on a not-running or repeatedly-restarting channel, at ADR-0015's levels. Deferred to **slice 2** — there is no channel for the gateway to lose until the cutover, so building the alarm now would be building it blind. Added as 9.12.
  Original: Check whether the gateway announces a dead channel as loudly as ADR-0015's restart-on-dead-updater did; if not, rebuild that alerting on top.
...
- [x] 8.9 **DONE for slice 1 (2026-08-02) — first of the two runs.** Sync the delta specs into `openspec/specs/` before archiving. **Now five modified and three new**, not four and three — `task-capture` was added 2026-08-01, see below. **Runs twice, once per slice** — 8.11 assigns every capability and, where one straddles, every requirement.

  **Deliberately not done yet, and that is correct.** `openspec/specs/` is the *current-state* baseline and this change is 122 of 190 tasks (56 of 181 when this was written; the count drifted and the eval caught it). Syncing now would assert the system already does things nobody has built. The CLAUDE.md warning is about not forgetting this at archive, not doing it early.
  ...
  **Slice 1's run, and what it actually put in the baseline.** Three new capability files, no modified ones — all five MODIFIED deltas went to slice 2, which is the tell that the boundary rule held: a modified capability describes a system that already exists and is changing, and none of those changes have happened.

  | Baseline file | Requirements synced | Held back for slice 2 |
  |---|---|---|
  | `openclaw-gateway-runtime` | 6 | 4 |
  | `claims-mcp-surface` | 5 | 3 |
  | `gmail-isolation-boundary` | 3 | 4 |

  The baseline is now **21 capabilities**, up from 18. `openspec validate --specs --strict` passes: 21 of 21.

  Each new file carries a **scope note in its `## Purpose`** naming what is deliberately absent and where it is specified. That is the part worth keeping when this is read later — a baseline capability holding three of seven requirements and no note reads as a complete description of a system, and the reader has no way to tell it is a third of one.

  The delta files' own `<!-- ... -->` split comments were stripped on the way in; they reference task numbers in a change that is about to be archived.

  **Not verified by this task:** that the synced requirements are true. The deploy, the preflight and the eval covered that; the sync only moved text those checks had already passed.

- [x] 8.11 **DECIDED (Justin, 2026-08-01): two slices.** Taken after 16.2 closed the last spike, which was his stated condition — slicing sensibly needed nothing left unknown. Original text: *Decide whether this change archives whole or in slices — after the section-0 spikes close. 190 tasks is a lot to hold un-synced. The cutover itself is atomic (one bot token, one poller), but the work before it is not: the internal API, the MCP surface and the preflight could ship and archive ahead of the transport swap, shrinking what has to be right on the day.*
  ...
  **How `openclaw-gateway-runtime`'s ten requirements split** — six become true at slice 1, four at slice 2. Recorded now because this is the reasoning that will be expensive to reconstruct at archive time:
  ...
  `gmail-isolation-boundary` **splits, correcting what this task first said** (eval, 2026-08-02). Slice 1 takes the requirements that are true once the containers run: no Gmail credential, no Google key, no mount reaching `app/data`, and a read-only tool inventory containing nothing dangerous. Slice 2 takes three that describe a system nobody has built — mail reachable *only* through named sweeps (the MCP surface has seven read tools and no sweeps), sweeps staying idempotent under gateway redelivery, and the drafts-only structural re-verification whose test is task 10.3, still open.

Recording the correction rather than quietly editing it, because the original "goes whole into slice 1" broke **this task's own rule**, stated four paragraphs above: *a requirement that would describe a system nobody has built yet cannot be in slice 1.* Writing the rule did not stop me breaking it in the same task, which is the argument for the eval existing. `telegram-bot`, `conversational-agent`, `llm-backend`, `reminder-scheduling` and `task-capture` go whole into slice 2; every one of them describes behaviour that only exists after the swap.
...
- [x] 8.14 **DONE 2026-08-02.** **Added by 8.11 — the mechanics of the split, to be done when slice 1's tasks are complete and not before.**
  ...
  **What was actually done, 2026-08-02.**

  - **Verbatim moves, no rewriting.** 66 open tasks and 9 completed ones moved by line range, so a cross-reference from slice 1 lands on the text it pointed at. Section numbers are therefore non-contiguous in both files (slice 1 jumps 2 → 7; slice 2 runs 3–6, 9, 10, 12) and that is deliberate — renumbering would break every reference in the ADRs and the design doc.
  - **Slice 1 ends with every task ticked.** Of the open items that were not cutover work, one remained: 13.6 went to `openspec/BACKLOG.md` with the reason it must not become a health gate.
  - **Five whole deltas moved with `git mv`** so history follows them: `telegram-bot`, `conversational-agent`, `llm-backend`, `reminder-scheduling`, `task-capture`.
  - **Three deltas split** — gateway-runtime 6/4, mcp-surface 5/3, gmail-isolation 3/4. Both halves carry a comment naming the other half and pointing here.
  - **`proposal.md` narrowed, not rewritten.** A scope banner at the top and `→ slice 2` markers on the seven bullets that moved. The original text stands underneath, because rewriting it to look as though the scope was always this would delete the reason the split exists.
  - **One correction found while doing it,** recorded in slice 2's `design.md` changelog rather than applied quietly: `gmail-isolation-boundary` splits **4/3, not 3/4**. The vision-OCR cap requirement's only scenario is a rematch sweep requested repeatedly, and slice 1's inventory has seven read tools and no sweeps — both pressure sources it names are slice-2 constructs, so 8.11's own boundary rule puts it there. That rule has now caught two assignments made against it, both on this same capability; the first was the eval's.
...
- [x] 16.6 **DONE — enabled; duplicate of the ticked 16.6 above.** Native rich messages are **off by default**: `channels.telegram.richMessages=true` enables "tables/details/rich media" (seen in `/status`). This is the feature 11.3 compares against Pillow cards — enable it before that comparison, or the test is unfair. A `command` button invokes a slash command, which maps onto the existing `/mark`, `/pet` surface with no token routing. If it holds, most of the bespoke callback bridge disappears.
...
- [x] 14.3 **RECORDED.** Note for whoever runs these commands from Git Bash on Windows: `/tmp/x` in a `docker exec` argument is rewritten to `C:/Users/.../Temp/x` by MSYS path translation, producing a misleading "not under an allowed directory" error naming a Windows path. Prefix with `MSYS_NO_PATHCONV=1`.
...
- [x] 18.6 **RECORDED in `app/openclaw/CLAUDE.md`'s `gateway-plugin/` row, and acted on rather than merely written down** — the preflight reads behaviour and the gateway's log, never `plugins list`. Original: **Gotcha to record in the module map:** `plugins list` / `plugins inspect` report `commands: []`, `hookCount: 0` and `Shape: non-capability` for a plugin whose commands demonstrably work. Those fields come from the **persisted registry**, not live runtime, and go stale silently (`persisted-registry-stale-policy`). Never diagnose a plugin from them; test the behaviour.
- [x] 18.7 **RECORDED, and it is THREE gates plus one loud one** (2026-08-02, from building the real plugin). Original said two. The full set: the entry must be enabled via `plugins.entries.<id>.enabled`; the default export must be wrapped in `definePluginEntry`; and `register()` must be **synchronous**, because the gateway discards its return value and an `async register()` loses every registration inside it. The fourth is the exception — a missing `openclaw.plugin.json` fails config validation **loudly**, which is the only gate in this area that announces itself. All four are in the module map's `gateway-plugin/` row. Original: **Two silent enablement gates** for a `plugins.load.paths` plugin: the entry must ALSO be enabled via `plugins.entries.<id>.enabled = true`, and the default export must be wrapped in `definePluginEntry` from `plugin-sdk/plugin-entry`. A plain object export loads without error and never runs. Neither failure produces a diagnostic.
...
- [x] 17.3 **DONE — the acceptance criterion has a number and the preflight enforces it** (`TURN_TOKEN_CEILING`, plus itemised component ceilings). **7.6 gains a second, harder justification.** Pinning the plugin set was a security requirement (`gmail-isolation-boundary`); it is now also a **feasibility** requirement. Measure tokens per turn after disabling unused plugins and pruning the command surface, and treat "turn size under the provider's TPM" as an acceptance criterion with a number, not an aspiration.
- [x] 17.4 **RESOLVED: Groq, measured not preferred.** The trimmed turn with the real claims inventory is ~4.9k against 12,000 TPM. **DECIDED by Justin, 2026-08-01: make the cuts, then measure, then choose — do not choose now.** The cuts (17.8/17.9) happen regardless because `gmail-isolation-boundary` wants the same tool allowlist. Groq stays the preference if the trimmed turn fits 12k TPM; Gemini is the fallback; a paid tier is the last resort even though he has said cost is covered. Decide the provider for the agent against that number, not against preference. Options: cut the surface until Groq's 12k TPM fits; use a provider with a higher TPM; or accept a paid tier. Note this is **per-minute**, a different constraint from ADR-0017's per-day budget — both now apply.
- [x] 17.5 **RECORDED in ADR-0009's amendment.** **Observed failover behaviour, relevant to ADR-0017 and D8.** OpenClaw retried the *same* model 3 times (10s/20s/30s backoff) then surfaced `decision=candidate_failed reason=rate_limit next=none`. Correct shape for a per-minute limit, and it confirms the gateway classifies Groq 429/413s as `rate_limit`. Two notes: with one model configured there is nothing to fall through to, and retrying a `413 request too large` is futile by construction — the request size never changes, so all 4 attempts were unsatisfiable.
  Original: **CONFIRMED THE HARD WAY, and it is not only Groq.** On 2026-08-01 the measurement turns exhausted **Gemini's** free tier (`429 ... exceeded your current quota`), and Gemini is the only vision-capable backend — so `extract_vision` was collateral damage from agent traffic, which is exactly the starvation this task predicted for Groq. The gateway now runs on Groq and its container holds no Google key at all (19b.5), which removes that particular collision but not the general one: agent chat and `invoice_matching`'s extraction still draw on the same Groq free tier. Give the gateway its own key, or accept the contention knowingly. Original: **Operational: the spike shares the production Groq key.** The gateway agent competes for the same free-tier quota as the live claims service. Give the gateway its own key, or accept that agent traffic can starve `invoice_matching`'s extraction calls.
...
- [x] 11.6 **DONE — duplicate of the ticked 11.6 above.** Record each verdict with its reason in `design.md`, including the ones where the bespoke version wins. A kept-because-measured decision and a kept-because-nobody-checked decision must not look the same later.
...
- [x] 19a.1 **DONE.** **Presentation payload shape.** Assert `gateway_client` emits buttons at `presentation.blocks[{type:"buttons"}]` and never at the top level. Five sends were silently discarded for this, each returning `ok:true` with a real message id. Assert the exact nesting, not just "buttons present".
- [x] 19a.2 **DONE** — the payload shapes the normalizer discards are refused before sending. **Never trust `ok`.** Assert `gateway_client` treats a success response with a dropped presentation as a failure where detectable, and that no code path infers "rendered" from "sent". This is the guard against the platform's defining hazard.
- [x] 19a.3 **DONE.** **Tool-inventory budget.** Assert the MCP inventory count stays at or under a declared maximum. Every tool schema ships on every chat turn; an unbounded inventory is a silent per-turn cost increase. Failing loudly on tool #N+1 is the point.
- [x] 19a.4 **DONE** — asserted in UTF-8 bytes, at the boundary. **Command payload length.** Assert every generated button command string stays under the transport limit, using the longest real case (a condition selection). Fail with the offending string, since Justin has accepted trimming to fit.
- [x] 19a.5 **One outbound seam.** Already implemented — keep `test_nothing_outside_gateway_client_shells_out_to_the_gateway` and extend it if a second transport appears.
- [x] 19a.7 **DONE.** **Tool inventory contains no filesystem, shell, browser, mailbox-search or secret-returning tool** (the `gmail-isolation-boundary` enforcement, currently task 2.3 — cross-referenced here so the NFR set is complete in one place).
...
- [x] 19b.1 **DONE** — itemised, fresh session key. **Agent turn size under a declared ceiling.** Measure with `openclaw agent --agent <id> --session-key <fresh> --message hi` and assert the result is under the configured model's per-minute limit. **Must use a fresh session key** — an accumulated session measures conversation history, not surface, and produced two false readings before this was caught.
- [x] 19b.2 **DONE.** **The model can actually serve a turn.** A model id that config accepts can still fail at runtime with `model_not_found` (Groq did, before a custom provider entry existed). Assert one real turn completes.
- [x] 19b.3 **DONE** — config *and* the running plugin set, since Telegram auto-enables without writing config. **Boundary plugins are disabled**: `browser`, `file-transfer`, and anything else granting filesystem, shell or browser reach. An upgrade re-enabling one must fail the deploy, not pass quietly.
- [x] 19b.4 **DONE.** **Access configuration**: `channels.telegram.dmPolicy == "allowlist"`, `allowFrom` non-empty, `commands.ownerAllowFrom` non-empty. Default is `pairing`, which hands unknown senders a live pairing code and the command to socially-engineer approval.
- [x] 19b.5 **DONE**, widened to any Google/Gemini credential. **The gateway holds no Gmail credential** and no Google key with a Gmail scope, and has no mount that can reach `app/data`.
...
- [x] 19b.7 **DONE** — including the `"any"` check. **Media outbox path is allowlisted** and is a narrow directory, not `app/data`.

### 19c. Documented, not tested — record where a test is impossible

- [x] 19c.1 **WRITTEN — `docs/gateway-deploy.md`.** How to deploy, why the gateway's secrets live in a separate `.env`, what each preflight check guards against, and — the point of the task — a **"What CANNOT be asserted, and why"** section that is longer than the check table.

  Seven entries, every one observed rather than imagined: a button command cannot be invoked by a deploy (no `openclaw command run`); a plugin cannot tell whether it owns the names it registered; nothing confirms a message *rendered* as sent, since `ok: true` accompanies a discarded payload; a failed caption edit is indistinguishable from a successful one in the gateway's own log; prompt-level rules are not enforceable and have failed twice; a Google key cannot be shown to lack a Gmail scope from outside; media roots are read from config rather than from the running process. Plus the standing gap that nothing here covers real load, concurrent taps or a restart mid-handler.

  Two of those carry a second-order warning worth keeping: the collision check depends on the gateway's log file, which lives **inside the container** and is lost on recreate — so a log rotation closes the window, and a format change turns the check into a silent pass. It has already been silently wrong once for exactly that reason.
