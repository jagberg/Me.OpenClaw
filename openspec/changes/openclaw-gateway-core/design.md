...
- ~~**Which model for the agent?**~~ **Answered 2026-08-01: Groq**, once the turn was cut from 22,810 to 3,865 tokens (ADR-0023). What remains open is the second half: **does the chain need verifying against the real tool schema?** ADR-0017 requires end-to-end verification of every model in a chain before it is relied on, and the gateway's chain is configured outside this repo — so that requirement currently has no owner. Sharpened by a live finding: a model id the gateway's config *accepts* can still fail at runtime with `model_not_found`, so "it validated" is not verification.
...
- **What replaced it is a budget, not slack.** ~6,600 tokens of headroom for the entire claims tool inventory. The platform's own 32 tools were five times that, so an inventory that looks reasonable can breach it.
- **The tool allowlist now serves two unrelated masters** — the isolation boundary needs it for security, the provider needs it for feasibility. A change made for one silently affects the other. See ADR-0023.
- **This platform fails silently** — **eight** distinct modes measured, the count having grown on every day spent with it. Success responses and inspection output are not evidence.
  - The seventh: a command button over 58 bytes is deleted from the keyboard, and if it was the only one the message arrives with no keyboard at all, `ok: true`.
  - The eighth is the worst of them because it is **partial**. A valid presentation sent through the CLI renders its buttons and silently drops its `text`/`context` blocks, so the message arrives looking deliberate rather than broken. Justin caught it by noticing a table was missing; nothing in the response, the logs, or the payload said so.
...
## 2026-08-01 — The token blocker is cleared; Groq stays the provider
**Decision:** Cut the turn to 5,355 tokens with `tools.allow` and authored workspace files. Groq free tier's 12,000 TPM now fits, so the agent stays on the same provider as extraction and vision.
**Reasoning:** 17.1 recorded the 12k ceiling as a hard blocker and 17.7 concluded a ~29k floor that pruning could not move. Both were wrong, and both were wrong the same way — a total read off a provider's error message, with no breakdown. `openclaw agent --json` itemises the turn; itemised, three-quarters of it was content the deployment chooses. Replacing the seeded workspace files took it to 20,616; a tool allowlist took it to 5,355.
