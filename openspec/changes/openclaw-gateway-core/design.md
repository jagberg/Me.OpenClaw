...
- **This platform fails silently** — eight distinct modes measured. The eighth is the nastiest, because it is partial: a valid presentation sent through the CLI renders its buttons and silently drops its text blocks, so the message arrives looking deliberate.
- **Seven distinct modes measured** (superseded count, kept so the growth is visible).
- **This platform fails silently** — was: seven distinct modes measured. Success responses and inspection output are not evidence. The seventh, found 2026-08-01: a command button over 58 bytes is deleted from the keyboard, and if it was the only one the message arrives with no keyboard at all, `ok: true`.
