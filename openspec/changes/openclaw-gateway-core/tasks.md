...
- [ ] 16.5 Identify which existing taps cannot become slash commands. Known risk: condition buttons deliberately carry an **index** rather than the text because condition text exceeds Telegram's 64-byte callback limit. Whether a `command` action has the same limit is unverified — if it does, condition selection still needs `callback` plus a plugin handler.
...
- [ ] 16.4 Re-test caption editing against a **document** (PDF) message. The passing test was a photo; the review alerts that most need `_append_result` are documents.
...
- [ ] 18.5 Confirm the command-payload size limit against the longest real button (`16.5`). Condition selection already uses an index rather than text, so `/setcond 7 3` style commands should fit comfortably — Justin has accepted trimming to fit if needed.
