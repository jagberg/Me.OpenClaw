...
- [x] 15.3 **Deprioritised by Justin, 2026-08-01: "don't worry about 15, it's not important."** Recorded rather than deleted, because 15.1 was his own finding and the behaviour is unchanged — a stranger who finds the bot still receives a live pairing code and the command to request approval. Accepting it is a decision, and a decision needs to be visible to whoever reads this next.
  What makes it defensible: approval is still a manual step only Justin can take, so disclosure alone grants nothing. What it costs: the app's current behaviour discloses nothing at all, so this is a real reduction, and the mitigating factor is Justin not approving a stranger — a human check rather than a technical one.
  19b.4 already asserts `dmPolicy == "allowlist"` at deploy, which is the control that matters most here and is unaffected by this decision. Reopen if the bot's username is ever published.
