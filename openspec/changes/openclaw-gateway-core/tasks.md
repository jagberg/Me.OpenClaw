...
- [ ] 13.3 **The stock agent has its own onboarding personality, and it talks to Justin unprompted.** Three consecutive replies in his chat asked him to name it, pick its "creature" type and its "vibe", choose an emoji, and confirm its pronouns, so it could write `IDENTITY.md`, `USER.md` and `SOUL.md` — driven by a `BOOTSTRAP.md` it says it must complete "before I can respond normally". This is a default, not something configured here. It must be turned off or fully pre-seeded before the gateway touches the real bot; a claims assistant that opens by asking Justin what to call itself is not the interface he built. Find whether these files can be shipped complete rather than interviewed for.
