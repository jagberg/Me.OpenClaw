# multi-pet-invoice-split Specification

## Purpose

One vet bill can cover more than one pet while a claim carries exactly one `pet_id`. This capability covers apportioning a single invoice's claimable amount between pets — one claim per pet, each with its own share — the shares Justin supplies, the guards that refuse a share it cannot prove, and what happens when one pet's insurer has no defined claim process. See ADR-0019 (and ADR-0007 for the ceiling rule it sits on).

The neighbouring case — one CHARGE paying several separate invoices, one per pet — is automatic and lives in `invoice-matching`, not here.
