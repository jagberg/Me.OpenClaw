# claimable-subtotal-provenance Specification

## Purpose
One accessor owns the question "what is this claim's claimable subtotal, and was it ever recorded?", so no caller can substitute the invoice total for an answer it does not have. `claim_status.claimable_subtotal` plus a self-checking guard test over `app/openclaw/*.py`.

The claimable subtotal is the line-item subtotal minus `NON_CLAIMABLE_KEYWORDS` and, for a per-pet split, this claim's own share (ADR-0007, ADR-0019). The invoice total is a different quantity. Five sites conflated them by convention until 2026-08-04, which is how a dismissed settlement flag came to tell Justin "we expected $430.74" — $580.74 − $150.00, computed from a number never submitted to Petcover as claimable.

See `app/openclaw/CLAUDE.md`'s collapse table (this is its sixth row) for why the guard test is part of the requirement rather than a nicety.
